...
export default {
  ...
  create: async (
    ...
    if (!body) {
      console.log("loginService", "nobody");
      return callback(null, "request without payload", errorMeta);
    }
    if (!body.idToken && !body.cookieToken)
      callback(null, "invalid request", errorMeta);
    if (body.idToken) {
      // CSR
      const decodedToken = await Auth.verifyIdToken(body.idToken);
    } else if (body.cookieToken) {
      // SSR
      try {
